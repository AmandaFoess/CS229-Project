...
def plot_ownership_durations(input_csv, output_folder):
    ...
    required_columns = ["Parcel Number", "Ownership Duration", "Duration Type", "Owner", "Zoning Code"]
    ...
    # Plot 6: Distribution of Zoning Codes by Duration Type
    print("Generating distribution of Zoning Codes by Duration Type...")
    plt.figure(figsize=(14, 8))
    sns.countplot(
        data=df,
        x="Zoning Code",
        hue="Duration Type",
        order=df["Zoning Code"].value_counts().index,
        palette="Set3"
    )
    plt.title("Distribution of Zoning Codes by Ownership Type")
    plt.xlabel("Zoning Code")
    plt.ylabel("Count")
    plt.xticks(rotation=45, ha="right")
    plt.grid()
    plt.savefig(f"{output_folder}/zoning_code_distribution_by_type.png")
    plt.show()
